...
CREATE TABLE T (x NUMERIC(10,3), y NUMERIC(10,3));
CREATE TABLE de_dust2 (x int, y int, z int, type char(10));
COPY T FROM :datafile DELIMITER ',';
COPY de_dust2 FROM :cs_data DELIMITER ' ';

SELECT MIN(x) as minx, MIN(y) as miny, MAX(x) as maxx, MAX(y) as maxy FROM T AS extremes;

SELECT heat_map(x,y USING PARAMETERS xbins=5, ybins=3, ymin='0', xmax='0', gaussian=True, bounding_box=True, normalize=3 ) OVER () AS (x, y, x_t, y_t, c) FROM T;

SELECT heat_map_image(x,y,c USING PARAMETERS outf='/tmp/testsizer.png') OVER () AS (img) FROM (SELECT heat_map(x,y USING PARAMETERS xbins=64, ybins=64, ymin='0', xmax='0', gaussian=False, bounding_box=True, normalize=3 ) OVER () AS (x, y, x_t, y_t, c) FROM T) AS heatmap LIMIT 50;

SELECT heat_map_image(x,y,c USING PARAMETERS flip_y=True, outf='/tmp/test3.png', inf='/tmp/testsizer.png') OVER () AS (img) FROM (SELECT heat_map(x,y USING PARAMETERS xbins=10, ybins=5, ymin='0', xmax='0', gaussian=False, bounding_box=True, normalize=3 ) OVER () AS (x, y, x_t, y_t, c) FROM T) AS heatmap LIMIT 50;
